'use client';

import { useState } from 'react';
import { getEndgameData } from '@/lib/api';
import type { EndgameSeason, EndgameFloor, EndgameMonster } from '@/lib/types';

const DATA = getEndgameData();

// 三種模式定義
const MODES = [
  { key: 'maze' as const, name: '忘卻之庭', en: 'Memory of Chaos', color: 'violet', desc: '月度重置，共 12 層，雙陣容通關' },
  { key: 'story' as const, name: '虛構敘事', en: 'Pure Fiction', color: 'sky', desc: '群體輸出為主，擊殺積分過關' },
  { key: 'boss' as const, name: '末日幻影', en: 'Apocalyptic Shadow', color: 'orange', desc: '單體爆發為主，BOSS 機制挑戰' },
] as const;

// 模式配色
const MODE_STYLES: Record<string, { border: string; badge: string; dot: string; tab: string; tabActive: string }> = {
  violet: {
    border: 'border-violet-500/30',
    badge: 'bg-violet-500/10 text-violet-400 border-violet-500/30',
    dot: 'bg-violet-400',
    tab: 'text-violet-400 border-violet-500/50 bg-violet-500/10',
    tabActive: 'border-violet-500/60 bg-violet-500/15',
  },
  sky: {
    border: 'border-sky-500/30',
    badge: 'bg-sky-500/10 text-sky-400 border-sky-500/30',
    dot: 'bg-sky-400',
    tab: 'text-sky-400 border-sky-500/50 bg-sky-500/10',
    tabActive: 'border-sky-500/60 bg-sky-500/15',
  },
  orange: {
    border: 'border-orange-500/30',
    badge: 'bg-orange-500/10 text-orange-400 border-orange-500/30',
    dot: 'bg-orange-400',
    tab: 'text-orange-400 border-orange-500/50 bg-orange-500/10',
    tabActive: 'border-orange-500/60 bg-orange-500/15',
  },
};

// 弱點屬性配色
const ELEMENT_COLOR: Record<string, string> = {
  火:   'bg-orange-500/20 text-orange-300 border-orange-500/40',
  冰:   'bg-sky-500/20    text-sky-300    border-sky-500/40',
  雷:   'bg-violet-500/20 text-violet-300 border-violet-500/40',
  風:   'bg-teal-500/20   text-teal-300   border-teal-500/40',
  量子: 'bg-purple-500/20 text-purple-300 border-purple-500/40',
  虛數: 'bg-yellow-500/20 text-yellow-200 border-yellow-500/40',
  物理: 'bg-gray-500/20   text-gray-300   border-gray-500/40',
};

// ── 日期格式化 ──────────────────────────────────────
function formatDate(dateStr: string): string {
  if (!dateStr) return '';
  const d = new Date(dateStr);
  if (isNaN(d.getTime())) return '';
  return `${d.getMonth() + 1}/${d.getDate()}`;
}

function getSeasonStatus(season: EndgameSeason): 'current' | 'ended' | 'upcoming' {
  if (!season.beginTime && !season.endTime) return 'upcoming';
  const now = Date.now();
  const end = season.endTime ? new Date(season.endTime).getTime() : Infinity;
  const begin = season.beginTime ? new Date(season.beginTime).getTime() : 0;
  if (now < begin) return 'upcoming';
  if (now > end) return 'ended';
  return 'current';
}

// ── 弱點標籤 ─────────────────────────────────────────
function WeaknessBadge({ element }: { element: string }) {
  return (
    <span className={`text-[10px] px-1.5 py-0.5 rounded border font-medium ${ELEMENT_COLOR[element] ?? 'bg-gray-500/20 text-gray-300 border-gray-500/40'}`}>
      {element}
    </span>
  );
}

// ── 怪物圖示 ─────────────────────────────────────────
function MonsterIcon({ monster }: { monster: EndgameMonster }) {
  const [failed, setFailed] = useState(false);
  return (
    <div className="flex flex-col items-center gap-1 w-20 shrink-0">
      <div className="w-16 h-16 rounded-lg overflow-hidden border border-white/10 bg-[#111125]">
        {!failed && monster.icon ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={monster.icon}
            alt={monster.name}
            className="w-full h-full object-contain"
            onError={() => setFailed(true)}
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center text-gray-700 text-xl">?</div>
        )}
      </div>
      <p className="text-center text-[10px] text-gray-400 leading-tight line-clamp-2 w-full">{monster.name}</p>
    </div>
  );
}

// ── 單個半場（上半/下半）─────────────────────────────
function HalfSide({ label, weakness, monsters }: { label: string; weakness: string[]; monsters: EndgameMonster[] }) {
  if (weakness.length === 0 && monsters.length === 0) return null;
  return (
    <div>
      <div className="flex items-center gap-2 mb-2">
        <span className="text-[11px] text-gray-500 font-medium">{label}</span>
        <div className="flex gap-1">
          {weakness.map(w => <WeaknessBadge key={w} element={w} />)}
        </div>
      </div>
      <div className="flex flex-wrap gap-2">
        {monsters.map((m, i) => <MonsterIcon key={i} monster={m} />)}
      </div>
    </div>
  );
}

// ── 樓層區塊 ─────────────────────────────────────────
function FloorCard({ floor }: { floor: EndgameFloor }) {
  const hasTwoSides = floor.weakness2.length > 0 || floor.monsters2.length > 0;
  return (
    <div className="border border-white/8 rounded-xl bg-white/[0.02] p-4">
      <p className="text-xs font-bold text-gray-400 mb-3">{floor.name}</p>
      <div className="flex flex-col gap-4">
        <HalfSide
          label={hasTwoSides ? '上半' : '敵方'}
          weakness={floor.weakness1}
          monsters={floor.monsters1}
        />
        {hasTwoSides && (
          <HalfSide
            label="下半"
            weakness={floor.weakness2}
            monsters={floor.monsters2}
          />
        )}
      </div>
      {/* BOSS 機制標籤 */}
      {(floor.tags1?.length || floor.tags2?.length) ? (
        <div className="mt-3 pt-3 border-t border-white/5 flex flex-wrap gap-1.5">
          {[...(floor.tags1 ?? []), ...(floor.tags2 ?? [])].map((tag, i) => (
            <span key={i} className="text-[10px] px-2 py-0.5 rounded-full border border-white/10 bg-white/5 text-gray-400" title={tag.desc}>
              {tag.name}
            </span>
          ))}
        </div>
      ) : null}
    </div>
  );
}

// ── 賽季卡片（可展開）──────────────────────────────
function SeasonCard({ season, modeColor, defaultOpen }: { season: EndgameSeason; modeColor: string; defaultOpen?: boolean }) {
  const [open, setOpen] = useState(defaultOpen ?? false);
  const status = getSeasonStatus(season);
  const style = MODE_STYLES[modeColor];

  // 忘卻之庭只顯示 F9–F12
  const displayFloors = season.floors.length === 12
    ? season.floors.slice(8) // F9–F12（index 8–11）
    : season.floors;

  return (
    <div className={`rounded-xl border ${style.border} overflow-hidden transition-colors`}>
      {/* 標題列 */}
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-white/[0.02] transition-colors"
      >
        {/* 展開箭頭 */}
        <span className={`text-gray-500 text-xs transition-transform ${open ? 'rotate-90' : ''}`}>▶</span>

        {/* 賽季名稱 */}
        <span className="text-white font-semibold text-sm flex-1 min-w-0 truncate">{season.name}</span>

        {/* 狀態標籤 */}
        {status === 'current' && (
          <span className="shrink-0 text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 font-semibold">
            當期
          </span>
        )}
        {status === 'upcoming' && (
          <span className="shrink-0 text-[10px] px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-400 border border-amber-500/30 font-semibold">
            測試
          </span>
        )}

        {/* 日期 */}
        {(season.beginTime || season.endTime) && (
          <span className="shrink-0 text-xs text-gray-600">
            {formatDate(season.beginTime)} ~ {formatDate(season.endTime)}
          </span>
        )}
      </button>

      {/* 展開內容 */}
      {open && (
        <div className="px-4 pb-4 space-y-3">
          {/* 本期增益 */}
          {season.buff && (
            <div className="rounded-lg bg-white/[0.03] border border-white/5 p-3">
              <p className="text-[11px] text-gray-500 mb-1">本期增益</p>
              <p className="text-xs text-gray-300 leading-relaxed whitespace-pre-line">{season.buff}</p>
            </div>
          )}

          {/* 可選增益（虛構敘事 / 末日幻影） */}
          {season.buffs && season.buffs.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {season.buffs.map((b, i) => (
                <div key={i} className="rounded-lg border border-white/8 bg-white/[0.02] px-3 py-2 flex-1 min-w-[200px]">
                  <p className="text-[11px] text-[#c9a227] font-semibold mb-0.5">{b.name}</p>
                  <p className="text-[10px] text-gray-400 leading-relaxed">{b.desc}</p>
                </div>
              ))}
            </div>
          )}

          {/* 樓層格子 */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {displayFloors.map((floor, i) => (
              <FloorCard key={i} floor={floor} />
            ))}
          </div>
        </div>
      )}
    </div>
  );
}

// ══════════════════════════════════════════════════════
export default function AbyssPage() {
  const [modeIdx, setModeIdx] = useState(0);
  const mode = MODES[modeIdx];
  const seasons = DATA[mode.key];

  return (
    <div className="space-y-5">
      {/* 頁面標題 */}
      <div>
        <h1 className="text-2xl font-bold text-white">終局挑戰</h1>
        <p className="text-sm text-gray-400 mt-1">忘卻之庭 · 虛構敘事 · 末日幻影</p>
      </div>

      {/* 模式切換 */}
      <div className="flex gap-2">
        {MODES.map((m, i) => {
          const s = MODE_STYLES[m.color];
          return (
            <button
              key={m.key}
              onClick={() => setModeIdx(i)}
              className={`px-4 py-2 rounded-lg text-sm border font-medium transition-colors ${
                modeIdx === i ? s.tab : 'border-white/10 text-gray-500 hover:text-gray-300 hover:border-white/20'
              }`}
            >
              {m.name}
            </button>
          );
        })}
      </div>

      {/* 模式說明 */}
      <div className="flex items-center gap-2 text-xs text-gray-500">
        <span className={`w-1.5 h-1.5 rounded-full ${MODE_STYLES[mode.color].dot}`} />
        <span>{mode.en}</span>
        <span className="text-gray-700">—</span>
        <span>{mode.desc}</span>
      </div>

      {/* 賽季列表 */}
      <div className="space-y-2">
        {seasons.map((season, i) => (
          <SeasonCard
            key={season.id}
            season={season}
            modeColor={mode.color}
            defaultOpen={i === 0}
          />
        ))}
      </div>
    </div>
  );
}
